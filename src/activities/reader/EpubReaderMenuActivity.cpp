...
std::vector<EpubReaderMenuActivity::MenuItem> EpubReaderMenuActivity::buildMenuItems(bool hasFootnotes,
  ...
  // One sync item per configured provider, hidden if no credentials.
...
void EpubReaderMenuActivity::render(RenderLock&&) {
  ...
  GUI.drawList(
      ...
      [this](int index) {
        // Respect overrideLabel (used by the per-provider sync items) when set.
        return menuItems[index].overrideLabel.empty() ? I18N.get(menuItems[index].labelId)
                                                       : menuItems[index].overrideLabel.c_str();
      },
      nullptr, nullptr,
      [this](int index) {
