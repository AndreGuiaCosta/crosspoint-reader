...
def partial_md5(path: Path) -> str:
    """Port of `partialMD5` in md5.ts — MD5 of up to 1 KiB at each offset below."""
    # Offsets are `step << (2 * i)` for i = -1..10 with step = 1024, evaluated
    # under JS int32 shift semantics. The i=-1 case is `1024 << -2` which JS
    # computes as `1024 << 30` (overflows int32 to 0), NOT 1024 >> 2 = 256
    # as a sign-aware port would compute. Verify with
    # `node -e "console.log(1024 << -2)"` → 0.
    OFFSETS = (0, 1024, 4096, 16384, 65536, 262144,
